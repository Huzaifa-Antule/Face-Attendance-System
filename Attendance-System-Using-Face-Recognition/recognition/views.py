...
def last_week_emp_count_vs_date():
	...
	df=pd.DataFrame()
	df["date"]=str_dates_all
	df["Std_count"]=emp_cnt_all
	

	
	
	sns.lineplot(data=df,x='date',y='Std_count')
...
def mark_your_attendance(request):
    detector = dlib.get_frontal_face_detector()
    predictor = dlib.shape_predictor(r'C:\Users\Huzaifa\Desktop\Project_SemIII\Attendance-Management-System-Using-Face-Recognition-main\Attendance-System-Using-Face-Recognition\face_recognition_data\shape_predictor_68_face_landmarks.dat')
    ...
    warning_displayed = False  # Flag to track if the warning message is displayed
            ...
            if pred != [-1]:
                ...
                if count[pred] == 4 and (time.time() - start[pred]) > 1.2:
                    ...
                    if not warning_displayed:
                        messages.success(request, f'{person_name}, Present ✅')
                ...
                if not warning_displayed:
                    messages.warning(request, f'Unknown Detected ❌, Please Contact with the Admin / Coordinator.')
                    warning_displayed = True  # Set the flag to True
